...
def initiate_subscan(
	...
	if not method:
		logger.warning(f'Task {scan_type} is not supported by ClaraRecon. Skipping')
		return
...
def subdomain_discovery(
	...
	for tool in tools:
		...
		if tool in default_subdomain_tools:
			...
			logger.warning(
				f'Subdomain discovery tool "{tool}" is not supported by ClaraRecon. Skipping.')
			continue
...
def send_file_to_discord(file_path, title=None):
	...
	webhook = DiscordWebhook(
		...
		username=title or "ClaraRecon Discord Plugin"
	)
